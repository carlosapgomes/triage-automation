...
- [ ] 1.1 Remover a rota `POST /callbacks/triage-decision` de `apps/bot_api/main.py` e limpar imports/dependências de callback HMAC vinculadas a decisão médica.
- [ ] 1.2 Remover o `build_widget_router` do wiring do `bot-api` e eliminar dependências exclusivas do fluxo widget HTTP de decisão.
