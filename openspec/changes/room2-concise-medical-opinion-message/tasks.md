...
- [x] 3.3 Atualizar testes de integração de postagem Room-2 para o novo contrato textual da mensagem `room2_case_summary`.
- [x] 3.4 Executar verificações obrigatórias do slice: `uv run pytest` (alvos), `uv run ruff check` (paths alterados), `uv run mypy` (paths alterados) e `markdownlint-cli2` (artefatos OpenSpec alterados).
