...
- [ ] 2.1 Criar testes de infraestrutura para garantir unicidade por `(room_id, window_start, window_end)` no controle de dispatch de resumo.
...
- [ ] 3.1 Escrever testes unitários para cálculo de janela determinística às 07:00 e 19:00 (intervalos de 12h, formato `[start, end)`).
- [ ] 3.2 Implementar serviço de scheduler que calcula a janela local e enfileira `post_room4_summary` com payload canônico em UTC.
...
- [ ] 5.1 Escrever testes de consulta/serviço para `pacientes recebidos`, `relatórios processados` e `casos avaliados` dentro da janela.
